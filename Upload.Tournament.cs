...
{
    public class UploadTournament : Upload
    {

        protected override XElement BuildUploadXML()
        {
            // Get and transform this effectively does basic format validation
            XElement results = this.GetWorksheetXML(0, HttpContext.Current.Server.MapPath(@"~\Xslt\Tournament\TournamentResults.xslt"));
            ...
            XElement transformed = XElement.Parse(Utility.Transform(results.ToString(), HttpContext.Current.Server.MapPath(@"~\Xslt\Tournament\Tournament.xslt"), xal));
            return transformed;
        }
        ...
        {
            // Validate with XSD
            XElement xsdErrors = this.ValidateXSD(item, HttpContext.Current.Server.MapPath(@"~\Xslt\Tournament\Tournament.xsd"));
            ...
            using (Persistence oDB = new Persistence())
            {
                ...
                oDB.Execute("Masters_ConnectionString", "ValidateTournamentData", parameters, out result);
            }
        ...
        protected override void Commit(XElement item)
        {
            ...
            using (Persistence oDB = new Persistence())
            {
                ...
                parameters.Add("InputXml");
                parameters[0].Value = item.ToString();
                oDB.Execute("Masters_ConnectionString", "SaveTournamentData", parameters);
            }
        }
